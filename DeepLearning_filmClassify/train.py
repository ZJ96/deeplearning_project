...
from util import AvgrageMeter, ConfuseMeter, accuracy
...
def validate(
    ...
    with torch.no_grad():  # 进行评测的时候网络不更新梯度
        ...
        for i, data in enumerate(validate_loader, 0):  # 0是下标起始位置默认为0
            ...
            if os.path.exists(tensorboard_path) == False:
                os.mkdir(tensorboard_path)
            writer.add_scalar('Validate/Loss', loss.item(), epoch*len(validate_loader)+i)
            writer.add_scalar('Validate/Accuracy', val_top1.avg, epoch*len(validate_loader)+i)
            writer.flush()
